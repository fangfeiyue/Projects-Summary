...
- Q:  商品详情页点击开通社员，跳转到开通社员页，开通社员后再返回商品详情页，开通社员的入口依然存在。查找原因是当返回到商品详情页只发送了options请求并没有发送所需的get请求

- A:  添加了一个中间页，点击开通社员先跳转到中间页再进入开通社员页，当返回的时候先返回到中间页再返回到商品详情页,这个时候就可以发送get请求了

- Q:  领券中心当头部滚动到一定距离后tab要悬浮到顶部，往下滚动到一定距离tab要恢复到原来的位置。当滚动过快tab会出现卡顿
...
- A:
  ...
  - 点击Add SSH key完成添加
#### 2018年08月17日 星期五
- Q: 做首页弹层优惠券时，样式没有按对象的形式使用，导致css样式没有前面加组件名，样式冲突

- A: 一是在webpack中进行配置；二是用对象的形式使用样式
